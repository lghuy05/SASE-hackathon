...
const USFDashboard = () => {
  ...
  const uploadLogo = async (file: File, company: string): Promise<string | null> => {
    if (!file) return null;
    const filePath = `logos/${company}_${Date.now()}`;
    const { data, error } = await supabase.storage.from('job-logos').upload(filePath, file, { upsert: true });
    if (error) {
      console.error('Logo upload error:', error);
      return null;
    }
    const { data: { publicUrl } } = supabase.storage.from('job-logos').getPublicUrl(data.path);
    return publicUrl;
  };
  ...
  const handleJobSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (error) {
        setJobError('Failed to post job: ' + (error.message || JSON.stringify(error)));
        console.error('Supabase insert error:', error);
      } else if (data && data.length > 0) {
        setJobPosts((prev) => [data[0] as JobPost, ...prev]);
  setJobForm({ title: '', company: '', location: '', job_type: '', industry: '', description: '', requirements: '', salary_range: '', hours_per_week: '', application_deadline: '' });
      }
    } catch (err: any) {
      setJobError('Unexpected error: ' + (err.message || JSON.stringify(err)));
  ...
  const PersonCard = ({ person }: { person: Person }) => (
    <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {person.tags && person.tags.map((tag: string) => (
